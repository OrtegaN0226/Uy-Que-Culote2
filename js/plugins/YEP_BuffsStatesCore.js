...
let gbccoffee_yepbuffstatecore_gamebattler_clearstates = Game_Battler.prototype.clearStates;
Game_Battler.prototype.clearStates = function() {
  if (!this._states) this._states = [];
  for (;;) {
    let state = this._states.shift();
    if (state) {
      this.customEffectEval(state, 'clearState');
    } else {
      break;
    }
  }
  gbccoffee_yepbuffstatecore_gamebattler_clearstates.call(this);
};
