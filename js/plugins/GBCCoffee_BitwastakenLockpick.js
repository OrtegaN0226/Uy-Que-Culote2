 ...
 * @type text[]
 * @desc Adding more items will equate to adding more lockpicking levels. The number you put there is the exp required for that level.
 * 
 * @help
 * SCRIPT CALLS: 
 * 
...
GBCCoffee.Bitwastaken.Lockpick = {
    ...
    lockpickExpVariableId: parseInt(PluginManager.parameters("GBCCoffee_BitwastakenLockpick")["lockpickExpVariableId"]),
    lockpickLevelSchema: JSON.parse(PluginManager.parameters("GBCCoffee_BitwastakenLockpick")["lockpickLvlSchema"]),
...
Game_Player.prototype.lockpickingLevel = function() {
    let level = 0;
    for (let i = 0; i < GBCCoffee.Bitwastaken.Lockpick.lockpickLevelSchema.length; i++) {
        let curLockpickExp = $gameVariables.value(GBCCoffee.Bitwastaken.Lockpick.lockpickExpVariableId);
        let requiredExp = parseInt(GBCCoffee.Bitwastaken.Lockpick.lockpickLevelSchema[i]);
        if (curLockpickExp >= requiredExp) level = i+1;
    }
    return level;
};
...
Window_Lockpick.prototype.updateMovingSprite = function() {
    ...
    if (this.tappedLeft) {
        ...
        if (this._movingSprite.x < maxX) {
            this._movingSprite.x = Math.min(this._movingSprite.x + (10*this.speedMultiplier), maxX);
        } else {
        ...
        if (this._movingSprite.x > minX) {
            this._movingSprite.x = Math.max(this._movingSprite.x - (10*this.speedMultiplier), minX);
        } else {
...
Window_Lockpick.prototype.setup = function(onSuccessEventId, onFailureEventId, speedMultiplier, targetWidth) {
    ...
    this.tappedLeft = true;
    this.tappedRight = false;
    this.speedMultiplier = speedMultiplier || GBCCoffee.Bitwastaken.Lockpick.speedMultiplier;
